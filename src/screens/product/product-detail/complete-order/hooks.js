...
const useCompleteOrderFacade = (navigation, t) => {
  ...
  const delivery = [{
    value: t('setting:text_by_hand'),
    name: 'by hand'
  },
  {
    value: t('setting:text_by_courier'),
    name: 'by courier'
  }]
  ...
  useEffect(() => {
    method == 'by hand' && setTotalPrice(parseFloat(price))
  }, [method])
  ...
  useEffect(() => {
    if (newFee) {
      if (transaction != 'donation' || method == 'by courier') {
        setTotalPrice(parseFloat(price) + parseFloat(newFee))
      }
    }
  }, [newFee])
  ...
  const _onChangeMethod = value => {
    if (value == t('setting:text_by_hand')) {
      setFee(0)
      setMethod(delivery[0].name)
    } else {
      setMethod(delivery[1].name)
    }
  }
  ...
  const _onHandleSubmit = () => {
    ...
    if ((duration != 0 && duration != '' && transaction == 'rent') || transaction == 'donation' || transaction == 'buy') {
      ...
      newItem.deliveryFee = method == 'by courier' && deliveryFee ? deliveryFee : 0
      newItem.distance = method == 'by courier' && buyerLocation ? calculateDistance() : null
