...
import { Header, ThemedView, LoadingView, IconOrder } from 'src/components';
...
export default function ProductDetail({ screenProps, navigation }) {
  const { t } = screenProps;
  const { productDetail, commentList, likesList, loading, currentUser, shouldShowComments,
   _onSendComment, _onDeleteComment, _onNavigateMessage, _onVerifyUser } = useProductDetailFacade(navigation, t);
  ...
  return (
    <ThemedView isFullView>
      ...
      {productDetail ? <KeyboardAwareScrollView
        ...
        <ListImages data={productDetail && productDetail.photos ? productDetail.photos : []} t={t} />
