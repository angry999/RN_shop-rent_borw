...
import { getListProduct, getListDesigners, likeItem, bookmarkItem, checkMessaging } from './actions';
...
import { HOME_PRODUCTS_LIMIT_ITEM, HOME_DESIGNERS_LIMIT_ITEM } from 'src/config/product';
...
const useHomeFacade = (navigation, t) => {
  ...
  const { products, nextPage, designers, scrollToTop, currentPageProduct } = useSelector(state => state.home)
  ...
  const navigateFromPushNotification = (remoteMessage) => {
    if (remoteMessage) {
      if (getNotificationType(remoteMessage.data.type) == 'product') {
